...
// ─── Password Rule ────────────────────────────────────────────────────────────
// 26.09.02 정책 변경: 대소문자 구분 없이 영문 최소 1자 + 숫자 최소 1자 조합을 필수로 하며,
// 특수기호는 선택적으로 포함할 수 있다(단, 작은따옴표(')는 사용 불가 — SQL 문자열 리터럴
// 구분자와 겹쳐 하위 시스템에서 오류를 유발할 수 있어 제외). 최소 8자 이상이어야 하며 길이
// 상한은 두지 않는다.

export const PASSWORD_MIN_LENGTH = 8

// 영문 1자 이상 + 숫자 1자 이상을 반드시 포함해야 하며, 공백과 작은따옴표(')를 제외한
// 출력 가능 ASCII 문자(영문 대소문자, 숫자, 특수기호)만 허용
export const PASSWORD_REGEX = new RegExp(
  `^(?=.*[a-zA-Z])(?=.*\\d)[\\x21-\\x26\\x28-\\x7E]{${PASSWORD_MIN_LENGTH},}$`
)

export const PASSWORD_RULE_MESSAGE =
  "비밀번호는 영문과 숫자를 포함하여 8자리 이상 입력해주세요. (특수문자 ' 는 사용할 수 없습니다)"

/** 비밀번호가 정책(대소문자 구분 없이 영문+숫자 조합 필수, 특수기호 선택("' " 제외), 8자 이상)을 만족하는지 검사한다 */
...
// 서버가 실제로 강제하는 한도(발송 24시간 5회, 인증 실패 24시간 5회)를 프론트에서도 동일하게
// 미러링해, 한도 초과가 확실한 요청은 API를 호출하기 전에 미리 막는다. 실제 방어는 서버 응답이
// 담당하며, 이 값들은 그 정책을 화면에 선제 반영하기 위한 보조 가드일 뿐이다.

export const EMAIL_SEND_LIMIT_MAX_ATTEMPTS = 5
export const EMAIL_SEND_LIMIT_WINDOW_MS = 24 * 60 * 60 * 1000

export const EMAIL_VERIFY_FAIL_LIMIT_MAX_ATTEMPTS = 5
export const EMAIL_VERIFY_FAIL_LIMIT_WINDOW_MS = 24 * 60 * 60 * 1000
